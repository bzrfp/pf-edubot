...
const int maxObjects = 1; // number of objects to retrieve
...
void followLine(bool approachingObject)
{
  ...
  // preferencia esquerda > centro > direito
  if (!lineLeft) // sensor esquedo leu 
  {
    moveLeft();
  }
  else if (!lineCenter) // sensor central leu
  {
    sparki.moveForward();
  }
  else if (!lineRight) // sensor direito leu 
  {
    moveRight();
  }
  else // ponto sem saída
  {
    sparki.moveRight(180); // roda o sparki
  }
